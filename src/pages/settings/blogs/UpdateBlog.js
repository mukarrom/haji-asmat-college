...
function UpdateBlog() {
	...
	const { id } = useParams();
	const [isLoading, error, data] = useGetData('blogs', id);
	...
	const handleFormSubmit = async (e) => {
				...
				fetch(`https://mmh.cyclic.app/api/v1/dynamic/${id}`, {
					...
					.then((updated) => {
						...
						if (updated.modifiedCount) {
							...
							if (file) {
								toast.success(
									'The blogs blog updated in database successfully'
								);
							}
							if (!file) {
							...
							toast.error('failed to update blogs blog in database');
						}
					});
			} else {
				toast.error(
					'No data found, Please insert data correctly or blog upload fail'
				);
			}
		}
		...
		// check blog file selected or not
		if (file) {
			...
			fetch(url, {
				...
				.then((imgData) => {
					// console.log(imgData);
					// check blog upload success or fail
					if (imgData.success) {
						setForm({
							...form,
							image: imgData.data.url,
						});
						// console.log({ success: true, url: img, message: 'blog upload in cloud success' });
						uploadToDb();
					} else {
						// goNext = false;
		...
		<div>
			...
			<div className="flex justify-center items-center lg:mx-6">
				...
				>
					...
					<div className="form-control w-full max-w-xs">
						<label className="label">
							<span className="label-text">Upload blog for blog</span>
						</label>
						<input
