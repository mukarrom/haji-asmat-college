				...
				fetch('https://mmh.cyclic.app/api/v1/dynamic/', {
					...
					.then((updated) => {
						...
						if (updated.insertedId) {
							...
							// setContent('');
							if (file) {
								toast.success('The events added in database successfully');
							}
							if (!file) {
								toast.success('texts updated in database successfully');
							}
						} else {
							toast.error('failed to Add event in database');
						}
					});
			} else {
				toast.error(
					'No data found, Please insert data correctly or event upload fail'
				);
			}
		}
		...
		<div>
			...
			<div className="flex justify-center items-center lg:mx-6">
				...
				>
					...
					<div className="form-control w-full max-w-xs">
						<label className="label">
							<span className="label-text">Upload image for event</span>
						</label>
						<input
