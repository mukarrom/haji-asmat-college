...
function UpdatePrincipal() {
	...
	const { id } = useParams();
	const [isLoading, error, data] = useGetData('principal', id);
	...
	const handleFormSubmit = async (e) => {
		...
		if (form) {
			...
			fetch(`https://mmh.cyclic.app/api/v1/dynamic/${data?._id}`, {
				method: 'PUT',
				headers: {
					dynamic: 'principal',
				...
				.then((principalData) => {
					...
					if (principalData?.modifiedCount) {
						// if (data?.modifiedCount) {
						toast.success('The principal updated in database successfully');
						// }
						// if (data?.result?.modifiedCount) {
						//     toast.success('The principal post  updated in database successfully');
						// }
					} else {
				...
				<form onSubmit={handleFormSubmit} className="">
					<div className="form-control w-full ">
						...
						<label className="label">
							<span className="label-text">Write voice of principal title</span>
						</label>
						<input
							name="title"
							type="text"
					...
					{/* <div className="form-control w-full"> */}
					{/*    <label className="label"> */}
					{/*        <span className="label-text">Write principal short description</span> */}
					{/*    </label> */}
					{/*    <textarea */}
					{/*        name="category" */}
					{/*        type="text" */}
					...
					{/* </div> */}
					{/* /!* >-------------->3. Date input end <--------------< *!/ */}
					{/* >-------------->5. principal Text Editor <--------------< */}
					<div className="form-control lg:col-span-2">
						<label className="label">
							<span className="label-text">Voice of principal detail</span>
						</label>
						<JoditEditor
							required
							// ref={editor}
