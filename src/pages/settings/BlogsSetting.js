	...
	const onSubmit = async (e) => {
		...
		function uploadToDb() {
			...
			if (e) {
				...
				fetch(`https://mmh.cyclic.app/api/v1/blogs/${whoActive}`, {
					method: 'PUT',
					headers: {
					...
					.then((data) => {
						if (data.success) {
							...
							if (data?.result?.upsertedCount) {
								toast.success(
									`The new ${whoActive} post  uploaded in database successfully`
								);
							}
							if (data?.result?.modifiedCount) {
								toast.success(
									`The ${whoActive} post  updated in database successfully`
								);
							}
						} else {
